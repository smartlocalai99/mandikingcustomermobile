...
export default function EmptyState({ icon = "fast-food-outline", imageSource, title, message, ctaLabel, onPressCta, style }) {
  return (
    <View style={[styles.wrapper, style]}>
      {imageSource ? <Image source={imageSource} style={styles.image} contentFit="contain" /> : (
        <View style={styles.iconWrap}><Ionicons name={icon} size={30} color={colors.textFaint} /></View>
      )}
...
const styles = StyleSheet.create({
  ...
  image: { height: 150, width: 190, marginBottom: 4 },
